...
function StartState:init()
    ...
    for i = 1, 64 do
        table.insert(positions, gFrames['tiles'][math.random(6)][math.random(6)])
    end
...
function StartState:update(dt)
    ...
    if not self.pauseInput then
        ...
        self.mouseX, self.mouseY = love.mouse.getPosition()
        local mouseVX, mouseVY  = push:toGame(self.mouseX, self.mouseY)

        if mouseVX > VIRTUAL_WIDTH / 2 - 25 and mouseVX < VIRTUAL_WIDTH / 2 - 25 + 50 and mouseVY > VIRTUAL_HEIGHT / 2 + 12 and mouseVY < VIRTUAL_HEIGHT / 2 + 41 then
            self.currentMenuItem = 1
        elseif mouseVX > VIRTUAL_WIDTH / 2 - 25 and mouseVX < VIRTUAL_WIDTH / 2 - 25 + 50 and mouseVY > VIRTUAL_HEIGHT / 2 + 41  and mouseVY < VIRTUAL_HEIGHT / 2 + 70 then
